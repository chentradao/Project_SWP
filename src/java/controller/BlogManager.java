    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            //delete a blog
                ...
                dao.deleteBlog(id);
                response.sendRedirect("BlogManager?service=listAllBlogs");
            }
        }
    }
...
private boolean isValidImageFile(String fileName) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.setCharacterEncoding("UTF-8"); // Đảm bảo dữ liệu gửi lên là UTF-8
    response.setContentType("text/html;charset=UTF-8"); // Đảm bảo phản hồi sử dụng UTF-8
        DAOBlog dao = new DAOBlog();
        String service = request.getParameter("service");
        ...
        if (service.equals("add")) {
            try {
                ...
                acc = (Accounts) session.getAttribute("acc");
                int blogAuthor;
                if(acc == null){
                    blogAuthor = Integer.parseInt(request.getParameter("blogAuthor"));
                }
                else blogAuthor = acc.getAccountID();
